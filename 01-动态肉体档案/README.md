...
1. **应用卡内补丁**：按 `patches/01-zod-schema.md` 和 `patches/02-mvu-update-rules.md` 的说明，把补丁内容粘到角色卡对应位置
...
| 原作者发新卡（V3.3 等） | 重新粘 2 处卡内补丁（位置锚点见 `patches/`）；CDN URL 不动，悬浮球自动用 fork 版 |

详细见 [`docs/复用迁移手册.md`](./docs/复用迁移手册.md)（首次正式迁移时编写）。

## 状态

- **当前版本**：v0.1.0-pre（设计阶段，补丁尚未应用到任何角色卡）
